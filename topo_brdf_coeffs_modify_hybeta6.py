...
#BAD_RANGE = [[300,430],[437,500],[510,1300],[1310,2600]]
...
def kernel_ndvi_outlier_search(band_subset_outlier, sample_k_vol, sample_k_geom,sample_c1, sample_cos_i, sample_slope, sample_ndvi, sample_topo_msk,sample_img_tag,idxRand_dict, hyObj_pointer_dict_list, image_smooth) :
    ...
    for iband in range(len(band_subset_outlier)):
      ...
      T_ = hierarchy.fcluster(H_s, 0.9, criterion='distance')
      print("Cluster thres 0.9",T_)    
    ...
    # Compute topographic and BRDF coefficients using data from multiple scenes
        ...
        for i,image in enumerate(args.img):
            ...
            if args.mask:
                ...
                hyObj.mask  =  (ndvi > NDVI_MIN_THRESHOLD) & (ndvi <= NDVI_MAX_THRESHOLD) & (ir != hyObj.no_data) & (ag_mask==0)  #(ndvi > .5) & (ir != hyObj.no_data) #(ndvi > .01) & (ir != hyObj.no_data)
            ...
            if hyObj.bad_bands[hyObj_dict[i].current_band]:
                ...
                if args.topo:    
                  if args.topo_sep==False:
                    #topo_coeff  = generate_topo_coeff_band(wave_samples,(wave_samples> REFL_MIN_THRESHOLD) & (wave_samples<REFL_MAX_THRESHOLD ) & sample_topo_msk ,sample_cos_i)
